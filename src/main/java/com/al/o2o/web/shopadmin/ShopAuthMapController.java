...
import com.al.o2o.dto.WechatInfo;
...
import com.al.o2o.util.ShortNetAddressUtil;
...
public class ShopAuthMapController {
    ...
    @Value("${wechat.prefix}")
    public void getUrlPrefix(String urlPrefix) {
        ShopAuthMapController.urlPrefix = urlPrefix;
    }

    @Value("${wechat.middle}")
    public void getUrlMiddle(String urlMiddle) {
        ShopAuthMapController.urlMiddle = urlMiddle;
    }

    @Value("${wechat.suffix}")
    public void getUrlSuffix(String urlSuffix) {
        ShopAuthMapController.urlSuffix = urlSuffix;
    }

    @Value("${wechat.auth.url}")
    public void getAuthUrl(String authUrl) {
    ...
    private void generateqrcode4shopauth(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (shop != null && shop.getShopId() != null) {
            ...
            try {
                ...
                BitMatrix qRcondig = CodeUtil.generateQRcodeStream(shortUrl, response);
                // 将二维码以图片流的形式输出到前端
                MatrixToImageWriter.writeToStream(qRcondig, "png", response.getOutputStream());
